...
where \(\mathrm{ClinVar}_w = 1.0\) (Pathogenic), 0.75 (Likely pathogenic), 0.5 (Uncertain significance), and 0.25 (Benign). A hyperparameter sensitivity analysis evaluated seven CADD:ClinVar ratios from 50:50 to 80:20 (Table~\ref{tab:weight_sensitivity}). All ratios produced near-identical rank ordering (Spearman \(\rho = 1.00\)) and AUC varied by less than 0.004. Cohen's \(d\) increased from 1.549 (50:50) to a plateau of 1.562 (70:30). The 60:40 configuration was selected as a principled compromise: a Cohen's \(d\) of 1.559 (\(p = 8.09 \times 10^{-15}\)) with a stronger CADD backbone than the 50:50 baseline, reducing dependence on ClinVar categorisation for variants with sparse clinical annotation.
...
\midrule
50:50  & 0.930 & 1.549 & \(4.82 \times 10^{-15}\) & 1.000 \\
55:45  & 0.927 & 1.554 & \(7.03 \times 10^{-15}\) & 1.000 \\
\textbf{60:40} & \textbf{0.926} & \textbf{1.559} & \(\mathbf{8.09 \times 10^{-15}}\) & \textbf{1.000} \\
65:35  & 0.926 & 1.561 & \(8.88 \times 10^{-15}\) & 1.000 \\
70:30  & 0.925 & 1.562 & \(1.02 \times 10^{-14}\) & 1.000 \\
75:25  & 0.922 & 1.562 & \(1.42 \times 10^{-14}\) & 1.000 \\
80:20  & 0.922 & 1.561 & \(1.48 \times 10^{-14}\) & 1.000 \\
\bottomrule
\end{tabular}
\end{table}
...
The ten chronological milestones correspond to the E-MTAB-6814 sampling points. Gastrulation and neurulation receive maximal weight because the three germ layers are established and the neural tube closes during these windows --- perturbations here cause the most widespread developmental consequences. 
...
\caption{Distribution of DevScore values for developmental disease genes (median = 12.3, IQR: 8.6--17.9) and adult-onset genes (median = 3.2, IQR: 2.3--5.2). The difference in central tendency reflects the contribution of the spatiotemporal criticality component C$_{\mathrm{stage}}$, which assigns lower weight to adult-peak genes.}
\label{fig:distributions}
\end{figure}

\begin{table}[h]
\centering
\caption{Classification performance of DevScore at the optimal threshold determined by Youden's J index. Metrics are from the full benchmark cohort (n=110; 60 developmental, 50 adult).}
...
\midrule
DevScore & 7.6 & 0.850 (51/60) & 0.940 (47/50) & 0.944 (51/54) & 0.839 (47/56) & 0.891 (98/110) \\
\bottomrule
\end{tabular}
}
\end{table}
...
When restricted to variants with resolvable CADD scores (n=110), DevScore maintained AUC 0.927, while CADD alone achieved only 0.457. The +0.470 AUC improvement demonstrates that CADD's molecular severity scoring, while useful, is insufficient for developmental context.

The Spearman correlation between DevScore and CADD was \(\rho = 0.131\) (p = 0.172), indicating that the two metrics capture orthogonal information. After controlling for gene class (developmental vs. adult), the partial Spearman correlation rose to \(\rho = 0.289\) (p = 0.002). This pattern -- a higher partial than raw correlation -- reflects a suppressor effect: the C\(_{\mathrm{stage}}\) component creates a systematic offset between the two classes (developmental genes score higher on DevScore than equivalently severe adult-onset genes), which compresses the overall correlation. Once this between-class offset is removed, a modest shared signal emerges from the common ClinVar and CADD severity information that both metrics incorporate.
...
OPTN  &  7.4 & 0.350 & 0.50 & 0.85 & 0.5 & 10.0 & ALS/glaucoma \\
SQSTM1&  7.2 & 0.578 & 0.50 & 0.50 & 0.5 & 25.2 & ALS/Paget disease \\
\bottomrule
\end{tabular}
}
\end{table}
...
\subsection{Developmental Timing as a Primary Axis of Pathogenicity}

The central finding of this study is that spatiotemporal gene expression context --- the developmental stage at which a gene reaches peak expression --- constitutes a primary axis of variant classification that existing tools entirely omit. DevScore's AUC of 0.927 against 0.457 (CADD), 0.446 (PolyPhen-2), and 0.397 (SIFT) demonstrates that the developmental dimension is a dominant signal, not a marginal refinement.
...
The weight-sensitivity analysis demonstrated robustness across seven CADD:ClinVar ratios (50:50 to 80:20): Spearman \(\rho = 1.00\), AUC variation below \(\pm 0.004\), and Cohen's \(d\) variation below 0.013 (Table~\ref{tab:weight_sensitivity}). The selected 60:40 configuration achieves Cohen's \(d = 1.559\) (\(p = 8.09 \times 10^{-15}\)) while retaining sufficient CADD backbone to buffer against ClinVar sparsity. The rank-order invariance across all tested weightings is critical for translational use: relative variant ranking is preserved regardless of the CADD:ClinVar ratio.
...
DevScore adds a biologically motivated developmental dimension absent from all major variant interpretation tools, achieving AUC 0.927 for discriminating developmental from adult-onset disease genes --- exceeding CADD by +0.470, PolyPhen-2 by +0.484, and SIFT by +0.533 --- while maintaining complete interpretability through its transparent four-component architecture. DevMutDB is freely available at \url{https://devmutdb.org} with a fully reproducible validation pipeline at \url{https://github.com/HaniDev/DevMutDB}. We believe DevScore represents a meaningful step toward developmentally-aware genomic medicine.

\section*{Acknowledgment}

The author thanks the developers and maintainers of the public genomic APIs used in this work: Ensembl VEP, CADD, ClinVar, gnomAD, and EMBL-EBI Expression Atlas. No specific funding was received for this work.
...
\midrule
  1  & SOX2    & 52.8  & 0.91  & 0.58  & 1.0  & 1.0  & 34.0   \\
  2  & PAX6    & 24.0  & 0.571 & 0.42  & 1.0  & 1.0  & 24.7   \\
  3  & TWIST1  & 9.2   & 0.539 & 0.36  & 0.95 & 0.5  & 22.6   \\
  4  & FGFR3   & 17.5  & 0.56  & 0.48  & 0.65 & 1.0  & 24.0   \\
  ...
  9  & KAT6A   & 18.7  & 0.585 & 0.32  & 1.0  & 1.0  & 25.7  \\
 10  & FOXP2   & 12.8  & 0.397 & 0.34  & 0.95 & 1.0  & 13.15  \\
 11  & CHD7    & 18.5  & 0.77  & 0.48  & 1.0  & 0.5  & 38.0   \\
 12  & KDM6A   & 9.2   & 0.665 & 0.29  & 0.95 & 0.5  & 31.0   \\
 13  & MED13L  & 8.6   & 0.552 & 0.31  & 1.0  & 0.5  & 23.5   \\
 14  & SETD5   & 7.6   & 0.54  & 0.28  & 1.0  & 0.5  & 22.7   \\
 15  & ADNP    & 18.5  & 0.543 & 0.34  & 1.0  & 1.0  & 22.9   \\
 16  & ANKRD11 & 12.7  & 0.74  & 0.36  & 0.95 & 0.5  & 36.0   \\
 17  & ARID1B  & 7.9   & 0.454 & 0.35  & 1.0  & 0.5  & 16.91  \\
 18  & KMT2D   & 9.9   & 0.546 & 0.38  & 0.95 & 0.5  & 23.1  \\
 19  & EP300   & 16.8  & 0.8   & 0.42  & 1.0  & 0.5  & 43.0   \\
 20  & CREBBP  & 17.6  & 0.8   & 0.44  & 1.0  & 0.5  & 43.0   \\
 21  & DYRK1A  & 22.4  & 0.621 & 0.36  & 1.0  & 1.0  & 28.1   \\
 22  & FOXG1   & 34.8  & 0.725 & 0.48  & 1.0  & 1.0  & 35.0   \\
 23  & MECP2   & 24.0  & 0.665 & 0.38  & 0.95 & 1.0  & 31.0   \\
 24  & PTEN    & 12.3  & 0.725 & 0.34  & 0.5  & 1.0  & 35.0   \\
 25  & RET     & 11.2  & 0.615 & 0.28  & 0.65 & 1.0  & 27.7   \\
 26  & SHH     & 11.1  & 0.528 & 0.42  & 1.0  & 0.5  & 21.9   \\
 27  & GLI3    & 8.7   & 0.483 & 0.36  & 1.0  & 0.5  & 18.88  \\
 28  & NOTCH3  & 6.1   & 0.581 & 0.42  & 0.5  & 0.5  & 25.4   \\
  29  & JAG1    & 11.0  & 0.68  & 0.34  & 0.95 & 0.5  & 32.0   \\
 30  & EYA1    & 10.0  & 0.362 & 0.29  & 0.95 & 1.0  & 10.78  \\
 31  & SALL1   & 4.5   & 0.366 & 0.26  & 0.95 & 0.5  & 11.04  \\
 32  & WT1     & 9.2   & 0.545 & 0.52  & 0.65 & 0.5  & 23.0   \\
 33  & PAX2    & 20.9  & 0.648 & 0.34  & 0.95 & 1.0  & 29.9   \\
 34  & ROBO3   & 2.0   & 0.595 & 0.22  & 0.3  & 0.5  & 26.3   \\
 35  & EFNB1   & 7.9   & 0.593 & 0.28  & 0.95 & 0.5  & 26.2   \\
 36  & FGF8    & 9.0   & 0.56  & 0.32  & 1.0  & 0.5  & 24.0   \\
 37  & OTX2    & 24.0  & 0.522 & 0.46  & 1.0  & 1.0  & 21.5   \\
 38  & VSX2    & 17.3  & 0.568 & 0.32  & 0.95 & 1.0  & 24.5   \\
 39  & FOXC1   & 13.6  & 0.397 & 0.36  & 0.95 & 1.0  & 13.14  \\
 40  & GJB2    & 4.2   & 0.585 & 0.48  & 0.3  & 0.5  & 25.7   \\
 41  & MYO7A   & 1.6   & 0.355 & 0.22  & 0.3  & 0.7  & 10.36  \\
 42  & CLN3    & 5.4   & 0.635 & 0.34  & 0.5  & 0.5  & 29.0   \\
 43  & HESX1   & 17.7  & 0.68  & 0.26  & 1.0  & 1.0  & 32.0   \\
 44  & LHX3    & 10.6  & 0.59  & 0.18  & 1.0  & 1.0  & 26.0   \\
 45  & PROP1   & 8.5   & 0.528 & 0.16  & 1.0  & 1.0  & 21.9   \\
 46  & DHCR7   & 18.1  & 0.695 & 0.52  & 1.0  & 0.5  & 33.0   \\
 47  & PKD1    & 9.8   & 0.519 & 0.58  & 0.65 & 0.5  & 21.3   \\
 48  & PDGFRB  & 15.7  & 0.71  & 0.34  & 0.65 & 1.0  & 34.0   \\
 49  & TRPV4   & 5.1   & 0.555 & 0.28  & 0.65 & 0.5  & 23.7   \\
 50  & RUNX2   & 9.4   & 0.615 & 0.32  & 0.95 & 0.5  & 27.7   \\
 51  & HOXD13  & 13.6  & 0.597 & 0.24  & 0.95 & 1.0  & 26.5   \\
 52  & SALL4   & 12.8  & 0.8   & 0.32  & 1.0  & 0.5  & 43.0   \\
 53  & STRA6   & 4.7   & 0.354 & 0.28  & 0.95 & 0.5  & 10.25  \\
 54  & KAT6B   & 17.8  & 0.603 & 0.31  & 0.95 & 1.0  & 26.9   \\
 55  & SMAD3   & 12.3  & 0.68  & 0.38  & 0.95 & 0.5  & 32.0   \\
 56  & FBN1    & 14.6  & 0.725 & 0.62  & 0.65 & 0.5  & 35.0   \\
 57  & COL1A1  & 27.6  & 0.611 & 0.68  & 0.95 & 0.7  & 27.4   \\
 58  & COL2A1  & 15.1  & 0.306 & 0.74  & 0.95 & 0.7  & 7.06   \\
...
\# & Gene & DevScore & V & E$_{\mathrm{peak}}$ & C$_{\mathrm{stage}}$ & D$_{\mathrm{domain}}$ & CADD \\
\midrule
  1  & BRCA1   & 6.4  & 0.94  & 0.274 & 0.25 & 1.0  & 36.0   \\
  2  & BRCA2   & 4.8  & 0.8   & 0.48  & 0.25 & 0.5  & 44.0   \\
  3  & APOE    & 5.9  & 0.471 & 1.0   & 0.25 & 0.5  & 18.07  \\
  4  & LRRK2   & 3.1  & 0.68  & 0.18  & 0.25 & 1.0  & 32.0   \\
  5  & SNCA    & 1.8  & 0.641 & 0.22  & 0.25 & 0.5  & 29.4   \\
  6  & HTT     & 2.0  & 0.506 & 0.32  & 0.25 & 0.5  & 20.4   \\
  7  & ATM     & 9.4  & 0.625 & 0.6   & 0.25 & 1.0  & 28.3   \\
  8  & MLH1    & 2.8  & 0.8   & 0.28  & 0.25 & 0.5  & 41.0   \\
  9  & MSH2    & 2.2  & 0.665 & 0.26  & 0.25 & 0.5  & 31.0   \\
 10  & APC     & 3.0  & 0.74  & 0.32  & 0.25 & 0.5  & 36.0   \\
 11  & VHL     & 2.2  & 0.631 & 0.28  & 0.25 & 0.5  & 28.7   \\
 12  & NF1     & 4.5  & 0.695 & 0.52  & 0.25 & 0.5  & 33.0   \\
 13  & NF2     & 2.3  & 0.77  & 0.24  & 0.25 & 0.5  & 38.0   \\
 14  & RB1     & 2.0  & 0.566 & 0.28  & 0.25 & 0.5  & 24.4   \\
 15  & TP53    & 8.2  & 0.71  & 0.46  & 0.25 & 1.0  & 34.0   \\
 16  & CDKN2A  & 0.7  & 0.328 & 0.18  & 0.25 & 0.5  & 8.53   \\
 17  & MEN1    & 1.8  & 0.638 & 0.22  & 0.25 & 0.5  & 29.2   \\
 18  & SDHB    & 2.9  & 0.725 & 0.32  & 0.25 & 0.5  & 35.0   \\
 ...
 26  & LDLR    & 4.7  & 0.551 & 0.68  & 0.25 & 0.5  & 23.4   \\
 27  & PCSK9   & 6.8  & 0.62  & 0.44  & 0.25 & 1.0  & 28.0   \\
 28  & F5      & 2.1  & 0.446 & 0.38  & 0.25 & 0.5  & 16.41  \\
 29  & HBB     & 5.1  & 0.408 & 1.0   & 0.25 & 0.5  & 13.85  \\
 30  & CFTR    & 4.1  & 0.637 & 0.52  & 0.25 & 0.5  & 29.1   \\
 31  & G6PD    & 6.6  & 0.578 & 0.46  & 0.25 & 1.0  & 25.2   \\
 32  & HEXA    & 3.4  & 0.56  & 0.24  & 0.25 & 1.0  & 24.0   \\
 ...
 38  & TGFBR2  & 5.7  & 0.71  & 0.32  & 0.25 & 1.0  & 34.0   \\
 39  & CACNA1A & 3.6  & 0.695 & 0.42  & 0.25 & 0.5  & 33.0   \\
 40  & PRKCG   & 4.1  & 0.583 & 0.28  & 0.25 & 1.0  & 25.5   \\
 41  & ATXN1   & 2.4  & 0.573 & 0.34  & 0.25 & 0.5  & 24.9   \\
 42  & DMPK    & 4.7  & 0.552 & 0.34  & 0.25 & 1.0  & 23.5   \\
 43  & SOD1    & 1.8  & 0.251 & 0.56  & 0.25 & 0.5  & 3.43   \\
 44  & FUS     & 2.9  & 0.555 & 0.42  & 0.25 & 0.5  & 23.7   \\
 45  & TARDBP  & 2.5  & 0.518 & 0.38  & 0.25 & 0.5  & 21.2   \\
 46  & OPTN    & 7.4  & 0.350 & 0.50  & 0.85 & 0.5  & 10.02  \\
 47  & VCP     & 4.0  & 0.569 & 0.50  & 0.28 & 0.5  & 24.6   \\
 48  & SQSTM1  & 7.2  & 0.578 & 0.50  & 0.50 & 0.5  & 25.2   \\
