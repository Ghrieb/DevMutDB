...
\author{%			
	Abdelkarim Hani Ghrieb\authorNumber{1,2}{*}
}
...
\address{%
	...
	\affiliation{{2}}{Department of Computer Science, University of the People \\ 595 E. Colorado Blvd., Suite 623, Pasadena, CA 91101, USA.
	}{
		{\email{*ghriebabdelkarimhani@gmail.com}}\\ 
	}
}
...
The architecture follows the growing methodological trend of integrating transcriptomic data into variant prioritization, as demonstrated by recent approaches combining expression data with genomic annotations \cite{bbag291,Genome_Res_2025_Jensen}. The fully integrated infrastructure is deployed in production at \url{https://devmutdb.vercel.app}, and the entire codebase is available under a restrictive license (commercial use, modification, distribution, and patent use prohibited), accessible via GitHub at \url{https://github.com/Ghrieb/DevMutDB}.
...
Integration of single-cell transcriptomic data from the Human Cell Atlas would enable tissue- and cell-type-resolved C\(_{\mathrm{stage}}\) weights, potentially improving resolution for genes with expression restricted to specific neuronal populations or organ compartments. Extension to non-coding variants could build on deep-learning frameworks such as MOSAIC \cite{bbag291} to incorporate developmental chromatin accessibility tracks and histone modification profiles. Population-adjusted E\(_{\mathrm{peak}}\) normalization, accounting for expression quantitative trait locus differences across gnomAD populations \cite{gnomad}, could improve equity of variant interpretation across diverse ancestries. Empirical calibration of C\(_{\mathrm{stage}}\) using Bayesian optimization on DECIPHER or DDD cohorts could transition from categorical to continuous constraint-based weights.
...
All data used in this study are derived from publicly available sources (Ensembl VEP, CADD, ClinVar, gnomAD, EMBL-EBI Expression Atlas). The validation benchmark dataset, analysis scripts, and full source code are available at \url{https://github.com/Ghrieb/DevMutDB} under a restrictive license.
